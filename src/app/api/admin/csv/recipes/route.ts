...
export async function GET(req: NextRequest) {
	...
	const csv = template ? recipeCsvTemplate() : await exportRecipesCsv();

	return csvResponse(
		csv,
		template ? 'modele-import-recettes.csv' : datedFilename('recettes-bmarket'),
	);
}
...
export async function POST(req: NextRequest) {
	...
	const report = await importRecipesCsv(await file.text(), mode);

	if (mode === 'commit' && report.errors.length === 0) {
